...
using System.Text;
using static NOF.Infrastructure.Extension.Authorization.Jwt.OAuthAuthorizationServerServiceHelpers;
...
public sealed class TokenHandler(
    ...
    IJwtAuthorityServiceClient jwtAuthorityServiceClient,
    ISigningKeyService signingKeyService,
    IOptions<JwtAuthorityOptions> jwtAuthorityOptions,
    ...
    : OAuthAuthorizationServerService.Token
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var validateResult = await jwtAuthorityServiceClient.ValidateJwtRefreshTokenAsync(
            new ValidateJwtRefreshTokenRequest { RefreshToken = request.RefreshToken },
            cancellationToken).ConfigureAwait(false);
        ...
        var revokeResult = await jwtAuthorityServiceClient.RevokeJwtRefreshTokenAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var accessClaims = profile.AccessTokenClaims
            .Select(static claim => new JwtClaim(claim.Key, claim.Value))
            .ToList();
        accessClaims.Add(new JwtClaim(OAuthClaimTypes.Scope, string.Join(' ', scopes)));

        var generateResult = await jwtAuthorityServiceClient.GenerateJwtTokenAsync(
            new GenerateJwtTokenRequest
            {
                ...
                RefreshToken = new JwtRefreshTokenOptions
                {
                    ...
                    Claims =
                    [
                        new JwtClaim(OAuthClaimTypes.Subject, subject),
                        new JwtClaim(OAuthClaimTypes.Scope, string.Join(' ', scopes))
                    ]
                }
            },
            cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var jwtToken = new JwtSecurityToken(
            issuer: jwtAuthorityOptions.Value.Issuer,
            audience: audience,
            claims: claims,
            notBefore: now,
