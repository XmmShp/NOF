...
public sealed class MemoryTransactionManager : ITransactionManager
{
    private readonly MemoryPersistenceStore _store;
    private readonly ILogger<MemoryTransactionManager> _logger;
    private readonly Stack<MemoryTransaction> _transactions = new();

    public MemoryTransactionManager(MemoryPersistenceStore store, ILogger<MemoryTransactionManager> logger)
    {
        _store = store;
        _logger = logger;
    }
    ...
    private Task CompleteAsync(MemoryTransaction transaction, bool commit, CancellationToken cancellationToken)
    {
    ...
    private sealed class MemoryTransaction : ITransaction
    {
        private readonly MemoryTransactionManager _manager;
        ...
        {
            ObjectDisposedException.ThrowIf(_disposed, typeof(MemoryTransaction));
            if (_completed)
            {
                return;
            }
