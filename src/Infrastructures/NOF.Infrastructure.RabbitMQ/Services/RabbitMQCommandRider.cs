...
public class RabbitMQCommandRider : ICommandRider
{
    ...
    {
        await PublishToRabbitMQAsync(command, headers, destinationEndpointName, cancellationToken);
    }

    private async Task PublishToRabbitMQAsync(ICommand command, IDictionary<string, string?>? headers, string? destinationEndpointName, CancellationToken cancellationToken)
    {
