...
public interface INOFAppBuilder : IHostApplicationBuilder
{
    ...
    /// </summary>
    /// <param name="config">The service configurator to register. Must not be null.</param>
    /// <returns>The current builder instance to allow method chaining.</returns>
    INOFAppBuilder AddServiceConfig(IServiceConfig config);
    ...
    where THostApplication : class, IHost
{
    ...
    /// These configurations are typically added via <see cref="AddServiceConfig"/> or delegate overloads,
    /// and are executed in dependency-aware order before the host application is constructed.
    /// </summary>
    protected readonly HashSet<IServiceConfig> ServiceConfigs;
    ...
    /// <inheritdoc />
    public virtual INOFAppBuilder AddServiceConfig(IServiceConfig config)
    {
    ...
    /// <inheritdoc />
    public virtual INOFAppBuilder RemoveServiceConfig(Predicate<IServiceConfig> predicate)
    {
