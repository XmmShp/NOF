...
public class ExposeToHttpEndpointMapperGenerator : IIncrementalGenerator
{
    ...
    private static void EmitEndpointMapping(StringBuilder sb, EndpointInfo ep)
    {
        ...
        if (isGet || !hasRouteParams)
        {
            ...
            sb.AppendLine();
            sb.AppendLine($"                async ({fromAttr} {requestType} request, [FromServices] NOF.Application.IRequestSender sender) =>");
            sb.AppendLine("                {");
        ...
        else
        {
            ...
            if (hasBody)
            {
                lambdaParams.Add($"[FromBody] {bodyDtoName} __body__");
            }
            lambdaParams.Add("[FromServices] NOF.Application.IRequestSender sender");
