...
public class MappableGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor MustBePartialStatic = new(
        id: "NOF021",
    ...
    private static readonly DiagnosticDescriptor OptionalSemanticMismatch = new(
        id: "NOF022",
    ...
    private static readonly DiagnosticDescriptor UnregisteredMapperFallback = new(
        id: "NOF023",
    ...
    private static bool GetNamedBoolArg(AttributeData attr, string name)
    {
        ...
        {
            if (arg.Key == name && arg.Value.Value is bool b) return b;
        }
        return false;
    }
        ...
        foreach (var group in grouped)
        {
            ...
            if (!first.IsPartialStatic)
            {
                spc.ReportDiagnostic(Diagnostic.Create(
                    MustBePartialStatic, first.Location, first.TypeName));
                continue;
            }
            ...
            foreach (var pair in allPairs)
            {
                ...
                {
                    spc.ReportDiagnostic(Diagnostic.Create(
                        DuplicateMapping, pair.Location, pair.SourceFullName, pair.DestFullName));
                    hasDuplicate = true;
                    continue;
                }
                seen[fwd] = pair;
                ...
                if (pair.TwoWay)
                {
                    ...
                    {
                        spc.ReportDiagnostic(Diagnostic.Create(
                            DuplicateMapping, pair.Location, pair.DestFullName, pair.SourceFullName));
                        hasDuplicate = true;
                    }
                    else
                    {
            ...
            if (hasDuplicate || validPairs.Count == 0) continue;
    ...
    {
        var srcFull = sourceType.ToDisplayString(FullyQualifiedFormat);
        var dstFull = destType.ToDisplayString(FullyQualifiedFormat);
    ...
    {
        if (destType is not INamedTypeSymbol namedType) return null;
        ...
        Location location, string propertyName)
    {
        ...
        {
            var destFull = destType.ToDisplayString(FullyQualifiedFormat);
        ...
        if (srcInnerFromOptional != null)
        {
            ...
            {
                spc.ReportDiagnostic(Diagnostic.Create(OptionalSemanticMismatch, location,
                    propertyName,
                    srcType.ToDisplayString(FullyQualifiedFormat),
                    destType.ToDisplayString(FullyQualifiedFormat)));
        ...
        if (srcInnerFromVo != null)
        {
            ...
            {
                var innerFull = srcInnerFromVo.ToDisplayString(FullyQualifiedFormat);
    ...
    private static string EmitMapperFallback(
        ...
        Location location, string propertyName)
    {
        var srcFull = srcType.ToDisplayString(FullyQualifiedFormat);
        var destFull = destType.ToDisplayString(FullyQualifiedFormat);

        if (!allAutoGeneratedPairs.Contains((srcFull, destFull)))
        {
            spc.ReportDiagnostic(Diagnostic.Create(UnregisteredMapperFallback, location,
    ...
    {
        if (SymbolEqualityComparer.Default.Equals(srcType, destType)) return false;

        // Implicit conversion (including user-defined) → no mapper
        var conv = compilation.ClassifyConversion(srcType, destType);
        if (conv.IsImplicit) return false;

        // User-defined explicit conversion → no mapper (will emit cast)
        if (HasUserDefinedConversion(srcType, destType)) return false;
        ...
        if (srcInnerOpt != null)
        {
            ...
            if (srcInnerIsNullable || !destIsNullable) return true; // semantic mismatch → mapper
        ...
        if (srcInnerResult != null)
        {
            ...
            if (srcResultInnerIsNullable || !destIsNullableForResult) return true; // semantic mismatch → mapper
        ...
        if (TryEmitPrimitiveConversion("_", srcType, destType, out _)) return false;
    ...
    private class MappingPairInfo
    {
        ...
        public MappingPairInfo(ITypeSymbol sourceType, ITypeSymbol destType, bool twoWay, Location location)
        {
            ...
            DestType = destType;
            SourceFullName = sourceType.ToDisplayString(FullyQualifiedFormat);
            DestFullName = destType.ToDisplayString(FullyQualifiedFormat);
            TwoWay = twoWay;
            Location = location;
        }
    }
