...
public sealed class TypeResolverTests
{
    ...
    {
        var typeName = Abstraction.TypeResolver.Register(typeof(LoadedAssemblyPayload));

        var type = Abstraction.TypeResolver.Resolve(typeName);
    ...
    {
        var type = Abstraction.TypeResolver.ResolveFromLoadedAssemblies(typeof(LoadedAssemblyPayload).FullName!);
    ...
    {
        var exception = Assert.Throws<InvalidOperationException>(() => Abstraction.TypeResolver.Resolve("NOF.Tests.MissingPayload"));
